...
public class ObesityAI : Enemy
{
    ...
    public GameObject ObesBullet;
    public float fireRate = 1.817f;
    ...
    void Start()
    {
        ...
        obesity = GetComponent<Transform>();
    ...
    private void FixedUpdate()
    {
        ...
        if (obesity.position.x > target.position.x)
        {
            if (facingRight)
            {
                obesity.localScale = new Vector3(transform.localScale.x * (-1), transform.localScale.y, transform.localScale.z);
                facingRight = false;
            }
        }
        if (obesity.position.x < target.position.x)
    ...
    IEnumerator InstantiateDelay()
    {
        ...
        Instantiate(ObesBullet, FirePos.position, Quaternion.identity);
    }
